                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    {user?.role === 'admin' && (
                      <button
                        onClick={async () => {
                          if (!confirm(`Are you sure you want to delete Packing Run #${run.id}?\n\nThis will remove the generated packets and add the bulk weight back.`)) return
                          const res = await window.api.packing.delete({ runId: run.id, userId: user.id })
                          if (res.ok) {
                            setSuccessMsg(`Packing run #${run.id} deleted.`)
                            loadHistory()
                            loadBase()
                          } else {
                            alert(res.error)
                        ...
                        style={{
                          ...
                          cursor: 'pointer', padding: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '4px'
                        }}
                        onMouseEnter={(e) => e.currentTarget.style.background = 'oklch(0.24 0.065 25)'}
                        onMouseLeave={(e) => e.currentTarget.style.background = 'none'}
                        title="Delete packing run"
                      >
